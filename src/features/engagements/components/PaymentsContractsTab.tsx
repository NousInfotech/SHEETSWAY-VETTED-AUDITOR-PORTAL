...
const PaymentsContractsTab: React.FC<PaymentsContractsTabProps> = ({ payments, contracts }) => {
  ...
  const handleUploadDeliverable = async (files: File[]) => {
    // TODO: Integrate with backend
    const newFiles = files.map((file) => ({
      name: file.name,
      url: URL.createObjectURL(file),
    }));
    setDeliverables((prev) => [...newFiles, ...prev]);
  };
  const handleDeleteDeliverable = (name: string) => {
    setDeliverables((prev) => prev.filter((file) => file.name !== name));
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="mb-4">
          <FileUploader onUpload={handleUploadDeliverable} multiple maxFiles={10} />
        </div>
        <div className="bg-card dark:bg-card border border-border rounded-lg overflow-hidden transition-colors">
          <table className="w-full">
            <thead className="bg-card dark:bg-card">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground bg-card dark:bg-card border-b border-border uppercase tracking-wider">Download</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground bg-card dark:bg-card border-b border-border uppercase tracking-wider">Delete</th>
              </tr>
            </thead>
            <tbody className="bg-card dark:bg-card divide-y divide-border">
              {deliverables.map(file => (
                <tr key={file.name} className="hover:bg-muted dark:hover:bg-card/80">
                  ...
                  <td className="px-6 py-4 text-sm">
                    <Button variant="destructive" size="icon" aria-label="Delete" onClick={() => handleDeleteDeliverable(file.name)}>
